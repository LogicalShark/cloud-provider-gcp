...
# tests with GCE-PD enabled, and tears it down.
...
export KOPS_CLUSTER_NAME="e2e-full-gcepd-kops.k8s.local"
...
# Run e2e tests with GCE-PD enabled, parallel, and skip regex
go test "${REPO_ROOT}/test/e2e/" \
  ...
  --ginkgo.args="--minStartupPods=8 --enabled-volume-drivers=gcepd"
